...
| 源类型 | 现有 `detectType` 必须为 `md`（`.md` / `.markdown`）。docx 当 source → `bad source`（那是 convert）。 |
...
| 中文 | 包内提交 SIL OFL CJK 字体（`packages/docforge/fonts/` + 许可文件）。pdf 与 docx **嵌入**该字体。运行时不下载、不读 CDN。 |
...
| 上限 | `stat.size > 800000` → `too large`（不按 UTF-8 读进内存）。读入去 BOM 后 JS 字符串 `.length > 200000` → `too large`。不静默截断。 |
...
Agent
  ...
  @flintloom/docforge
        ...
        └─ html|docx|pdf → parseBlocks → writer（pdfkit / docx，嵌入包内字体）
        │
  writeFile(out) 覆盖；失败不改已有 out
        │
...
`generateDocument` 只负责读源、按 `absOut` 扩展名写入（覆盖）。**不做** hidden / 工作区闸门 / 父目录检查；那些在工具层。源 `detectType(absSource)` 不是 `md` → 抛 `Error("bad source")`。`formatFromOutRelPath(absOut)` 为空 → 抛 `Error("bad out")`。读源超限 → 抛 `Error("too large")`。库失败 → 抛 `Error("unreadable")`。工具把这些 `message` 变成 `failed: <message>`。md 拷贝失败（I/O）同样 `unreadable`。
...
6. `stat(source)`：ENOENT → `failed: not found`；不是文件 → `failed: not a file`
7. `detectType(source)` 不是 `md` → `failed: bad source`
8. `formatFromOutRelPath(outRel)` 为空 → `failed: bad out`
9. `source` 的 `stat.size > 800000` → `failed: too large`
10. `dirname(out)`：不存在或不是目录 → `failed: missing parent`
11. `out` 已存在且不是文件 → `failed: not a file`
12. 调用 `generateDocument`；抛错 → 若 `message` 是第 7 节理由表里的短英文则 `failed: <message>`，否则 `failed: unreadable`（不把系统路径或 stack 回给模型），**不写 / 不改** `out`
13. 成功 JSON
...
- docx：npm `docx`，标题用 Heading，列表用 bullet/number，代码用等宽段落，表用 Table。嵌入 `fonts/` 下那份 CJK 字体。
- pdf：`pdfkit`，从左上往下流式排版。正文字号 12，heading 更大。嵌入同一字体；代码块也用它（不换系统 Courier）。无页眉页脚。不加载网络。
...
9. writer 失败：`failed: unreadable`，预先放好的 `out` 字节不变。
