...
范围：总 spec 第三刀的 **`doc_summarize` 块**。工作区文档 parse 成 markdown 后，由工具内层调用 `chat.stream` 生成短摘要。挂在现有 `@flintloom/docforge` 的 `apply` 上，yml 不新加插件。禁止再往 `createRuntime` 里 `register`。本片不改 loop / session 事件、不扩展 `ToolExec`、不写盘、不改 A2UI catalog、不改 Files preview `kind`。
...
| 内层请求 | `chat.stream({ messages, tools: [] }, exec.signal)`。两参数 API；`signal` 不进 `ChatRequest`。`messages` 只有 system + user，**不**带外层 `deriveMessages()`。 |
| system 原文 | 见第 5.1 节常量 `SUMMARIZE_SYSTEM`。禁止改字。 |
| chunk | 只拼接 `type === "text"`。`tool_call` 忽略、不执行。若整段流没有任何 text → `unreadable`。`type === "error"` 立即 `unreadable`，丢弃已拼文本，不把 `message` 回给外层。 |
| 写盘 | **不写**。 |
...
- 新 session 事件；扩展 `ToolExec`；给 `ChatProvider` 加 `complete()`
...
内层 `stream` 不写入 session。外层只有一次 `tool/call` + 一次 `tool/result`（JSON 或 `failed:` / `aborted`）。
...
8. 迭代 chunk：`text` 拼接；`tool_call` 忽略；`error` → `unreadable`。抛错时若 `signal.aborted` → `aborted`，否则 `unreadable`。
9. 循环结束后若 `signal.aborted` → `aborted`。
10. 拼接结果长度为 0 → `unreadable`。
11. 返回 `{ ok: true, summary: joined.slice(0, SUMMARIZE_MAX_CHARS) }`。
...
`failFromError` 不新增理由。`ModelKindMissingError` 在 `summarizeDocument` 内已映射为 `unreadable`，不得把其 `message` 原样回给模型。
...
yml 去掉 docforge 的 host 测试同时断言 schema 无 `doc_summarize`。

## 6. 数据流

1. Boot：yml 加载 docforge → 现有工具 + `doc_summarize`。此时可以没有 chat。
2. `tool_call` → 路径闸门 + `tools/pre-execute` → parse → `resolveChat` → 内层 `stream`（无 session 事件）→ `tool/result`（JSON 或失败串）。
3. 下一 Agent step 的投影只含该 `tool/result`，不含内层 user markdown。
4. 不写 sidecar、不入库。Files 预览不变。CLI 可调同一工具。
...
| 缺 chat、stream 抛错、`error` chunk、无 text | `failed: unreadable` |
...
- 内层 `tools: []`；忽略 `tool_call`，不在内层执行工具。
- 全文不进 `tool/result`、不进 session log。

## 9. 测试

使用已提交的 `packages/docforge/tests/fixtures/hello.md` 与 `binary.bin`。不提交新二进制夹具。`ChatProvider` 一律假对象；不打真实网。失败路径断言 `stream` 调用次数为 0。

1. `hello.md`：假 stream 产出 `"Short summary."` → 键顺序 `status`、`path`、`summary`；`path === "hello.md"`；`summary === "Short summary."`。假 chat 收到的 user 含 `# Hello` 与 `发展`；system 等于 `SUMMARIZE_SYSTEM`；`tools` 为 `[]`。
2. 空 `.md` → `failed: empty text`，stream 未调用。
3. 正文以 `failed: empty text\n# Hello` 开头的 md 仍走 stream（禁止 `startsWith("failed:")`）。
4. `binary.bin` → `failed: unsupported type`，stream 未调用。
5. 缺 `path`、hidden、缺文件、目录、`stat.size > GENERATE_MAX_BYTES`、规范化后超 `GENERATE_MAX_CHARS`。
6. 未 `registerChat` 的 `ModelRegistry` → `failed: unreadable`，stream 未调用（对一份非空 md）。
7. 假 stream 产出长度 `4001` 的串 → `summary.length === 4000`，且不含 `[truncated]`。
8. 只有 `tool_call` chunk → `unreadable`。`error` chunk → `unreadable`。
9. stream 中 abort → `"aborted"`。
10. `apply` 含 `doc_summarize`；yml 去掉 docforge 则不含。`stop()` 后不含。无 models 插件时 apply 抛 `models`。
11. host `src` 无 `createDocSummarizeTool`。loop / session / desktop 不 import `summarizeDocument`。
12. 现有 parse / generate / convert / edit / compare / ingest / 预览 / 信息图 / A2UI 保持绿。
