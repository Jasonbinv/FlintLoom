...
33. 本地 llama overlay（同上 §33）— `127.0.0.1` / `localhost` 的 `BASE_URL` 仅 overlay `models-chat`，不挂媒体与 guard。
34. 媒体 / guard 独立 env（同上 §34）— `FLINTLOOM_MEDIA_*` / `FLINTLOOM_GUARD_*` 可与本地 chat 并存。
35. 桌面凭据与通道设置（见 [凭据与通道设计](2026-08-23-flintloom-desktop-credentials-channels-design.md)）— `~/.flintloom/credentials` per-slot API、Settings 页、reload runtime；本地 guard 指引；Telegram 可配。
