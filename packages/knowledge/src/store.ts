...
INSERT INTO documents (workspace_root, rel_path, title, status, fail_reason, ingested_at, body)
VALUES (?, ?, ?, ?, ?, ?, ?)
...
`);
  ...
  const ftsSearchStmt = fts
    ? db.prepare(`
SELECT d.id, d.workspace_root, d.rel_path, d.title, d.body
...
SELECT id, workspace_root, rel_path, title, body
FROM documents
