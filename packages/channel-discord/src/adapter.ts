...
import { sendDiscordText } from "./outbound.ts";
import { lastAssistantText } from "./text.ts";

const SESSION_PREFIX = "discord:";

function discordChannelId(sessionId: string): string | undefined {
...
export function createDiscordAdapter(ctx: Context, parsed: DiscordConfig): ChannelAdapter {
  ...
  return {
    ...
    async send(outbound) {
      const channelId = discordChannelId(outbound.sessionId);
      if (channelId === undefined) {
        throw new Error("bad sessionId");
      }
      await sendDiscordText(parsed, channelId, outbound.text, outbound.signal);
    },
    async deliver(outbound) {
      ...
      await sendDiscordText(parsed, channelId, text, outbound.signal);
    },
  };
}
