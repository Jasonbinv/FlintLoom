...
import { sendFeishuText } from "./outbound.ts";
import { lastAssistantText } from "./text.ts";

const SESSION_PREFIX = "feishu:";

function feishuChatId(sessionId: string): string | undefined {
  if (!sessionId.startsWith(SESSION_PREFIX)) {
    return undefined;
  }
  const raw = sessionId.slice(SESSION_PREFIX.length);
  return /^oc_[\w-]+$/.test(raw) ? raw : undefined;
}

export function createFeishuAdapter(ctx: Context, parsed: FeishuConfig): ChannelAdapter {
  ...
  return {
    ...
    async send(outbound) {
      const chatId = feishuChatId(outbound.sessionId);
      if (chatId === undefined) {
        throw new Error("bad sessionId");
      }
      await sendFeishuText(parsed, chatId, outbound.text, outbound.signal);
    },
    async deliver(outbound) {
      ...
      await sendFeishuText(parsed, chatId, text, outbound.signal);
    },
  };
}
