...
describe("antv-infographic skill", () => {
  it("routes SWOT, relation, and chart families to official templates and infographic_render", () => {
    ...
    expect(skill).toContain("infographic_render");
    expect(skill).toMatch(/Do not call `a2ui_emit`/);
  ...
  it("keeps chat output on infographic_render instead of HTML or box-line JSON", () => {
    expect(skill).toContain("infographic_render");
